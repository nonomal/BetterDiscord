...
export default function request() {
    ...
    if ("method" in options && methods.indexOf(options.method.toLowerCase()) >= 0) {
        return BetterDiscord.HttpManager[options.method](url, options, callback);
    }

    return BetterDiscord.HttpManager.default(url, options, callback);
};
...
Object.assign(request, Object.fromEntries(
    methods.concat(Object.keys(aliases)).map(method => [method, function () {
        ...
        return BetterDiscord.HttpManager[aliases[method] || method](url, options, callback);
    }])
));